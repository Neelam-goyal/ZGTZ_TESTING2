...
********Item level data***************
    IF wa IS NOT INITIAL.
      SELECT
       ...
       b~documentdate
     FROM i_operationalacctgdocitem AS a
     INNER JOIN i_journalentry AS b ON a~AccountingDocument = b~AccountingDocument
     WHERE a~ClearingAccountingDocument = @cleardoc  AND  "AND a~ClearingDocFiscalYear = '2024' AND a~CompanyCode = 'GT00' AND
