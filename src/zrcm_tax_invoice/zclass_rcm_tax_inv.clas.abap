...
CLASS ZCLASS_RCM_TAX_INV IMPLEMENTATION.
  ...
  METHOD read_posts.
    ...
    LEFT JOIN i_regiontext AS c ON b~Region = c~Region
    LEFT JOIN i_countrytext AS d ON b~Country = d~Country
    LEFT JOIN i_accountingdocumentjournal AS e ON a~AccountingDocument = e~AccountingDocument AND a~CompanyCode = e~CompanyCode
    AND a~FiscalYear = e~FiscalYear
    LEFT JOIN i_companycode AS f ON a~CompanyCode = f~CompanyCode
    AND a~FiscalYear = e~FiscalYear
    WHERE a~AccountingDocumentType IN ( 'RE', 'KR' ) AND a~FinancialAccountType = 'K'
    AND a~AccountingDocument EQ @accountingDocNo
...
*    out->write( wa_head ).
    ...
    b~purchaseorderdate
    FROM i_operationalacctgdocitem AS a
    LEFT JOIN i_purchaseorderapi01 AS b ON a~PurchasingDocument = b~PurchaseOrder
...
*    out->write( tax_porcb ).
   ...
   d~regionname
   FROM i_operationalacctgdocitem AS a
   LEFT JOIN i_plant AS b ON a~BusinessPlace = b~Plant
   LEFT JOIN i_kr_businessplace AS c ON a~BusinessPlace = c~branch
   LEFT JOIN i_regiontext AS d ON a~IN_GSTPlaceOfSupply = d~region
   WHERE a~CompanyCode = 'GT00'
   AND a~AccountingDocumentType IN ( 'RE', 'KR' ) AND a~FinancialAccountType = 'K'
   AND a~AccountingDocument EQ @accountingDocNo
   INTO @DATA(wa_head_bill).
    ...
    a~taxitemgroup,
     ...
     LEFT JOIN i_accountingdocumentjournal AS c ON a~GLAccount = c~GLAccount
*     WHERE a~AccountingDocumentItemType = 'W' AND a~TransactionTypeDetermination = 'WRX' or a~CostElement is NOT INITIAL
     WHERE a~AccountingDocument EQ @accountingDocNo
